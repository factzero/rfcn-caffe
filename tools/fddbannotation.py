...
imagesdir=rootdir+"/VOCfddb/JPEGImages"
annotationdir=rootdir+"/FDDB-folds"
labelsdir=rootdir+"/VOCfddb/labels"
Annotationsdir=rootdir+"/VOCfddb/Annotations"
convert2rects=True
bsavexmlanno=True
bsavetxtanno=True
datasetprefix="/media/zero/data/01.DataSet/fddb/VOCfddb/JPEGImages/"
...
def generatetxt(trainratio=0.7,valratio=0.2,testratio=0.1):
    ...
    ftrain=open(rootdir+"/VOCfddb/"+"train.txt","w")
    fval=open(rootdir+"/VOCfddb/"+"val.txt","w")
    ftrainval=open(rootdir+"/VOCfddb/"+"trainval.txt","w")
    ftest=open(rootdir+"/VOCfddb/"+"test.txt","w")
    index=0 
...
def generatevocsets(trainratio=0.7,valratio=0.2,testratio=0.1):
    if not os.path.exists(rootdir+"/VOCfddb/ImageSets"):
        os.mkdir(rootdir+"/VOCfddb/ImageSets")
    if not os.path.exists(rootdir+"/VOCfddb/ImageSets/Main"):
        os.mkdir(rootdir+"/VOCfddb/ImageSets/Main")
    ftrain=open(rootdir+"/VOCfddb/ImageSets/Main/train.txt",'w')
    fval=open(rootdir+"/VOCfddb/ImageSets/Main/val.txt",'w')
    ftrainval=open(rootdir+"/VOCfddb/ImageSets/Main/trainval.txt",'w')
    ftest=open(rootdir+"/VOCfddb/ImageSets/Main/test.txt",'w')
    files=os.listdir(labelsdir)
    for i in range(len(files)):
